...
def _get_file_and_routing_context(file_id: str, user_id: str):
    ...
    if file_record["file_type"] == "google_sheets":
        try:
            ...
            raise HTTPException(status_code=401, detail="Google account not connected or token expired")
        return {
            "type": "google",
...
def get_sheet_range(file_id: str, sheet_name: str, range: str, current_user: dict = Depends(get_current_user)):
    ctx = _get_file_and_routing_context(file_id, current_user["user_id"])
    if ctx["type"] == "google":
        res = sheets_service.read_range(ctx["identifier"], ctx["access_token"], sheet_name, range)
    else:
...
def run_operation(file_id: str, req: OperationRequest, current_user: dict = Depends(get_current_user)):
    ctx = _get_file_and_routing_context(file_id, current_user["user_id"])
    ...
    if ctx["type"] == "google":
        ...
        if func:
            ...
            raise HTTPException(status_code=400, detail="Operation not supported for Google Sheets")
    else:
        func = getattr(excel_service, op_name, None)
        if func:
            res = func(ctx["identifier"], **params)
        else:
            raise HTTPException(status_code=400, detail="Operation not supported for Excel")
