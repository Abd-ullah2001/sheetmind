...
async def create_webhook(
    config: WebhookConfigCreate, current_user: dict = Depends(get_current_user)
):
    """
...
async def delete_webhook(
    ...
    """
    app.database.supabase_client.table("webhook_configs").delete().eq(
        "id", webhook_id
...
async def inbound_webhook(request: InboundWebhookRequest):
    ...
    try:
        result = run_agent_query(
            user_id=request.user_id, file_id=request.file_id, query=request.query
        )
        return result
