...
async def google_callback(params: OAuthCallbackParams):
        ...
        if params.access_token:
            ...
            expires_at = (
                datetime.datetime.fromtimestamp(
                    params.expires_at, tz=datetime.timezone.utc
                )
            ...
            if not params.code or not params.redirect_uri:
                raise HTTPException(
                    status_code=422,
                    detail="code and redirect_uri or access_token are required",
                )
    ...
    store_oauth_tokens(
        user["id"], "google", access_token, refresh_token, expires_at, scopes
    )
...
async def microsoft_callback(params: OAuthCallbackParams):
    if params.access_token:
        ...
        refresh_token = params.refresh_token
        expires_at = (
            datetime.datetime.fromtimestamp(params.expires_at, tz=datetime.timezone.utc)
            if params.expires_at
            else datetime.datetime.now(datetime.timezone.utc)
            + datetime.timedelta(seconds=3600)
        )
        ...
        if "error" in result:
            raise HTTPException(
                status_code=400,
                detail=f"Failed to exchange MS token: {result.get('error_description')}",
            )
    ...
    store_oauth_tokens(
        user["id"], "microsoft", access_token, refresh_token, expires_at, scopes
    )
...
def logout(current_user: dict = Depends(get_current_user)):
    app.database.supabase_client.table("oauth_tokens").delete().eq(
        "user_id", current_user["user_id"]
    ).eq("provider", current_user["provider"]).execute()
