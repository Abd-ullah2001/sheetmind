...
def upsert_user(email: str, name: str, avatar_url: str | None, provider: str) -> dict:
    res = (
    ...
    if len(users) > 0:
        user = users[0]
        app.database.supabase_client.table("users").update({"last_login": "now()"}).eq(
            "id", user["id"]
        ).execute()
        return user
    else:
...
def store_oauth_tokens(
    ...
    app.database.supabase_client.table("oauth_tokens").upsert(
...
def get_oauth_tokens(user_id: str, provider: str) -> str:
    res = (
        app.database.supabase_client.table("oauth_tokens")
        .select("*")
        .eq("user_id", user_id)
...
def refresh_google_token(user_id: str, refresh_token: str | None) -> str:
    import httpx
...
def refresh_microsoft_token(user_id: str, refresh_token: str | None) -> str:
    import msal
    ...
    if "access_token" not in result:
        raise ValueError(
            f"Failed to refresh MS token: {result.get('error_description')}"
        )
