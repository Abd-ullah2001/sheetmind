...
export function FileUploadZone({ token }: { token?: string }) {
  ...
  return (
    <label
      className="flex min-h-40 cursor-pointer flex-col items-center justify-center rounded-[16px] border border-dashed border-[var(--color-iris-edge)] bg-[var(--color-pure-white)] p-6 text-center transition-colors hover:bg-[var(--color-lavender-wash)]/50"
      onDragOver={(event) => event.preventDefault()}
      onDrop={(event) => { event.preventDefault(); const file = event.dataTransfer.files.item(0); if (file) void upload(file); }}
    >
      {isUploading ? <LoadingSpinner className="h-6 w-6" /> : <UploadCloud className="h-8 w-8 text-[var(--color-aubergine-core)]" />}
      <div className="mt-3 text-[14px] font-semibold text-[var(--color-midnight-plum)]">Drop an Excel workbook here</div>
      <div className="mt-1 text-[14px] text-[var(--color-steel)]">or click to choose a .xlsx file</div>
